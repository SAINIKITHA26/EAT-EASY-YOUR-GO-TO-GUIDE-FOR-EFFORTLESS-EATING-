...
if(isset($_POST["addcart"])){
    if(isset($_SESSION["cart"])){
        $food_id_array = array_column($_SESSION["cart"], "food_id");
        if(!in_array($_GET["food_id"], $food_id_array)){
            $index = count($_SESSION["cart"]);
                $item= array('food_id' => $_GET['food_id'], 
                                'f_name' => $_POST['f_name'],
        ...
        $item=$arrayName = array('food_id' => $_GET['food_id'], 
                                 'f_name' => $_POST['f_name'],
                                'quantity' => $_POST['quantity'],
                                'price' => $_POST['price']
                                );
  ...
  <path fill-rule="evenodd" d="M15.854 8.354a.5.5 0 0 0 0-.708l-3-3a.5.5 0 0 0-.708.708L14.293 7.5H5.5a.5.5 0 0 0 0 1h8.793l-2.147 2.146a.5.5 0 0 0 .708.708l3-3z"/>
    ...
    <div class="cover-chome2">
        ...
        <div class="login-title">
            <h1 class="header">PRODUCT <span class="cover_span">VIEW</span></h1>
        </div>
        <!-- breakfast -->
        
        <div class="menu-box2" id="breakfast">
            ...
            <div class="">
               ...
               <form action='{$_SERVER["REQUEST_URI"]}' method='POST'>
                <div class='menu-flex-col' style='gap:8px;'>
                    <img src='uploadImage/{$row["image"]}' alt='food' width='250px' height='150px'>
                    <span class='menu-title'>{$row["f_name"]}</span>
                    <span class='menu-title'>{$row["shop"]}</span>
                    <span class='menu-title2'>{$row["location"]}</span>
                    <span class='menu-title3'>{$row["description"]}</span>
                    <span class='menu-title2'>{$row["price"]}</span>
                    <span style='color:black;'>Enter Quantity <input type='number' class='inputfield' name='quantity' required></span>
                    <input type='hidden' name='f_name' value='{$row["f_name"]}'>
                    <input type='hidden' name='price' value='{$row["price"]}'>
                    <input type='submit' class='menu-btn' name='addcart' value='add to cart'>
